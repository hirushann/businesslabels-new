...
export default function CartPageClient({ popularProducts = [] }: { popularProducts?: ProductCardData[] }) {
  ...
  const shipping = items.length === 0 ? 0 : (isFreeShipping ? 0 : (defaultRule ? defaultRule.shipping_cost : 0));
    ...
    <>
      <div className="px-4 md:px-8 lg:px-10 py-12">
        <div className="max-w-360 mx-auto w-full">
          ...
          <h1 className="text-[32px] font-bold text-ink mb-10 text-center md:text-center">
            {t('cart.title')}
          </h1>

          <div className="w-full grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,24rem)] items-start">
            {/* Left Panel: Products List */}
            <div className="w-full min-w-0 bg-white rounded-xl border border-line overflow-hidden shadow-[2px_4px_20px_rgba(109,109,120,0.06)] relative flex flex-col">
                ...
                {mainItems.map((item, index) => {
                  ...
                  return (
                    <div key={item.key} className="flex flex-col gap-3">
                      ...
                      <div className="w-full grid grid-cols-1 lg:grid-cols-[minmax(16rem,1fr)_minmax(6rem,0.28fr)_minmax(12rem,0.42fr)_minmax(6rem,0.22fr)] lg:items-center gap-4 lg:gap-6 py-2">
                        ...
                        <div className="flex items-start md:items-center gap-3 md:gap-6 min-w-0">
                          ...
                          <button
                            ...
                            className="w-4 text-subtle hover:text-[#EF4444] transition-colors flex justify-center shrink-0"
                          ...
                          <div className="flex-1 flex items-start md:items-center gap-3 md:gap-4 min-w-0">
                            ...
                            <div className="flex-1 flex flex-col gap-1 min-w-0">
                              ...
                              <h3 className="text-lg font-bold text-ink leading-tight min-w-0">
                                {href ? (
                                  <Link href={href} className="hover:text-brand transition-colors block whitespace-normal break-words">
                                    {item.name}
                                  </Link>
                                ) : (
                            ...
                            {item.isLabelProduct && item.packingGroup && item.packingGroup > 0 ? (
                              <>
                                ...
                                <button
                                  ...
                                  className="h-[38px] px-4 rounded-[100px] border border-brand text-brand text-base font-semibold hover:bg-brand-soft transition-colors whitespace-nowrap"
                                >
                      ...
                      {linkedWarranty ? (
                        <div className="ml-0 md:ml-10 lg:ml-0 w-full md:w-auto p-3 lg:px-0 bg-surface rounded-[12px] grid grid-cols-1 gap-4 md:grid-cols-[minmax(10rem,1.1fr)_minmax(5.5rem,0.5fr)_minmax(5rem,0.7fr)_minmax(5rem,0.45fr)] lg:grid-cols-[minmax(16rem,1fr)_minmax(6rem,0.28fr)_minmax(12rem,0.42fr)_minmax(6rem,0.22fr)] md:items-start lg:gap-6">
                          <div className="flex flex-col gap-1.5 min-w-0 lg:pl-10">
                            <span className="text-base font-semibold text-ink leading-tight">
                              {linkedWarranty.name}
                            </span>
                            {warrantyTypeName ? (
                              <span className="text-xs font-semibold text-subtle leading-[1.3]">
                                {t.has('cart.warrantyType') ? t('cart.warrantyType') : 'Warranty type'}: {warrantyTypeName}
                              </span>
                            ) : null}
                            {linkedWarranty.warranty?.description ? (
                              <span className="text-sm font-normal text-copy">
                                {linkedWarranty.warranty.description}
                              </span>
                            ) : null}
                          </div>
                          ...
                          <div className="w-full flex flex-col justify-center items-start gap-1">
                            <span className="text-lg md:text-xl font-semibold text-ink">
                          ...
                          <div className="text-base font-semibold text-ink leading-5">
                            {t('cart.years', {
                          ...
                          <div className="w-full flex flex-col justify-center items-start md:items-end gap-1.5 lg:pr-3">
                            ...
                            <button
                              ...
                              className="text-danger text-sm font-normal underline hover:text-red-700 transition-colors"
                            >
                              {t('common.remove')}
                            </button>
                          </div>
              ...
              {/* Subtotal Footer */}
              <div className="w-full border-t border-line px-6 py-4 bg-white flex justify-end items-center gap-6 rounded-b-xl">
                <span className="text-lg font-normal text-ink">
                  {t('cart.subtotal')}:
                </span>
            ...
            <aside className="w-full bg-white rounded-xl border border-line overflow-hidden shadow-[2px_4px_20px_rgba(109,109,120,0.06)] flex flex-col">
              {/* Header */}
              ...
              <div className="p-6 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4">
                  {/* Delivery Time Banner — hidden per UI requirement */}
                  {false && (
                  ...
                  <div className="flex items-center gap-2">
                    <div className="shrink-0">
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.666 15.0026V5.0026C11.666 4.56058 11.4904 4.13665 11.1779 3.82409C10.8653 3.51153 10.4414 3.33594 9.99935 3.33594H3.33268C2.89065 3.33594 2.46673 3.51153 2.15417 3.82409C1.84161 4.13665 1.66602 4.56058 1.66602 5.0026V14.1693C1.66602 14.3903 1.75381 14.6022 1.91009 14.7585C2.06637 14.9148 2.27834 15.0026 2.49935 15.0026H4.16602" stroke="var(--copy)" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round"/>
                        <path d="M12.5 15H7.5" stroke="var(--copy)" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round"/>
                        ...
                        <path d="M14.1667 16.6693C15.0871 16.6693 15.8333 15.9231 15.8333 15.0026C15.8333 14.0821 15.0871 13.3359 14.1667 13.3359C13.2462 13.3359 12.5 14.0821 12.5 15.0026C12.5 15.9231 13.2462 16.6693 14.1667 16.6693Z" stroke="var(--copy)" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round"/>
                        <path d="M5.83268 16.6693C6.75316 16.6693 7.49935 15.9231 7.49935 15.0026C7.49935 14.0821 6.75316 13.3359 5.83268 13.3359C4.91221 13.3359 4.16602 14.0821 4.16602 15.0026C4.16602 15.9231 4.91221 16.6693 5.83268 16.6693Z" stroke="var(--copy)" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </div>
                    <p className="text-base font-normal text-copy leading-tight">
                  ...
                  {totalAmount < shippingThreshold ? (
                    <div className="w-full p-3 bg-gradient-to-br from-[var(--brand-soft)] to-white rounded-xl border border-[#FFEDD4] flex items-center gap-2">
                      <div className="shrink-0">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <p className="text-[#BB8F06] text-base font-normal leading-tight flex-1">
                ...
                <div className="w-full p-4 bg-gradient-to-br from-[var(--brand-soft)] to-white rounded-xl border-2 border-[#FFEDD4] flex flex-col gap-3">
                  ...
                  <div className="relative w-full h-[52px]">
                    ...
                    >
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <mask id="mask0_cart_purchase_ref" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="0" y="0" width="20" height="20">
                          <rect width="20" height="20" fill="#D9D9D9"/>
                        </mask>
                        <g mask="url(#mask0_cart_purchase_ref)">
