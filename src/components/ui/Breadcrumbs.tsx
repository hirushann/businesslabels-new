    ...
    <nav className={`flex flex-wrap items-center gap-2 text-sm ${className}`} aria-label="Breadcrumb">
      ...
      {items.map((item, index) => (
        <div key={index} className="flex items-center gap-2">
          ...
          {item.href ? (
            <LocaleLink href={item.href} className="hover:text-brand transition-colors">
              {item.label}
            </LocaleLink>
          ) : (
